...
function cs_profile_reviews_html($uid, $cs_directory_per_page){
	...
	if($count_post > $cs_directory_per_page){ 
		...
		if ( function_exists( 'cs_pagination' ) ) { echo cs_pagination($count_post, $cs_directory_per_page,$qrystr); }
	}
}
//===================================
// @ get ad reviews
...
function cs_get_ad_reviews($cs_post_id = ''){
	if($cs_post_id){
		...
		if(isset($rating)){$agg_rating = $rating;} else {$agg_rating = 0;}
		 echo '
                            ...
                            ?>
                                <article class="saved-ads ads-in holder-<?php echo intval($post->ID); ?>">
                                    ...
                                    <div class="text">
                                        ...
                                        <a data-toggle="tooltip" data-placement="top" title="<?php _e('Unfavourite','directory');?>" onclick="javascript:cs_delete_wishlist('<?php echo esc_url(admin_url('admin-ajax.php'));?>','<?php echo intval($post->ID);?>')" class="tolbtn close close-<?php echo intval($post->ID); ?>"><i class="icon-star-o"></i></a>
                                    </div>
                                </article>
                     <?php
                        endwhile;
    	...
    	<ul class="cs-user-menu">
        	<li class="active">
                <a href="<?php echo esc_url($cs_login_page_link); ?>">
                    <i class="icon-user2"></i><?php _e('About My Organization','directory'); ?>
                </a>
            </li>
            <li>
                <a href="<?php echo esc_url($cs_login_page_link); ?>">
                    <i class="icon-briefcase4"></i><?php _e('My Projects','directory'); ?>
                </a>
            </li>
            <!-- <li>
                <a href="<?php echo esc_url($cs_login_page_link); ?>">
    ...
    <ul class="cs-user-menu">
       ...
       <?php
            if ( is_user_logged_in() and $current_user->ID == $uid) {?>
        	 <li <?php if($action  == 'my_ads'){ echo 'class="active"'; } ?>>
            ...
            <!--<li <?php if($action  == 'saved_ads'){ echo 'class="active"'; } ?>>
               ...
               <?php 
                ...
                <li <?php if(isset($_GET['action']) && $_GET['action'] == 'payments'){ echo 'class="active"'; } ?>>
                    <a href="<?php echo cs_user_admin_profile_link($cs_page_id, 'payments', $uid); ?>">
                        <i class="icon-credit-card"></i><?php _e('Payments','directory');?>
                    </a>
                </li>
                <li <?php if(isset($_GET['action']) && $_GET['action'] == 'user-reviews'){ echo 'class="active"'; } ?>>
						...
						if(isset($package_id) && $package_id <> ''){
							...
							if ( $package_id == '0000000000' ) {
								$custom_fields .= '<ul class="dr_userinfo">';
									$custom_fields .= '<li><label>Title: </label>Free</li>';
									$custom_fields .= '<li><label>Price: </label>0</li>';
									$custom_fields .= '<li><label>Duration: </label>UNLIMITED</li>';
								$custom_fields .= '</ul>';
								...
								if ( isset($dir_pkg) && is_array($dir_pkg) && count($dir_pkg)>0 ) {
									...
									if(isset($dir_pkg['package_title']))
										$custom_fields .= '<li><label>Title: </label><span>'.$dir_pkg['package_title'].'</span></li>';
									if(isset($dir_pkg['package_price']))
										$custom_fields .= '<li><label>Price: </label><span>'.$dir_pkg['package_price'].'</span></li>';
									if(isset($dir_pkg['package_duration']))
										$custom_fields .= '<li><label>Duration: </label><span>'.$dir_pkg['package_duration'].' no of days.</span></li>';
...
function cs_add_transection( $uid, $post_id ,$cs_directory_pkg_names, $payment_date, $addType, $is_featured, $cs_dir_type_price, $gateway ='', $cs_directory_type='' ) {
	...
	if( isset( $cs_packages_options ) && is_array( $cs_packages_options ) && count( $cs_packages_options ) > 0 ){
		...
		$cs_package_price	= $dir_pkg['package_price'];
		$cs_new_price		= '';
		$cs_totalPrice		= $cs_package_price;
		
		if ( isset ( $is_featured ) && $is_featured == 'yes' && $is_true == true ) {
		...
		/*if ( isset( $cs_new_price ) && $cs_new_price > 0 ){
			$cs_totalPrice	= $cs_new_price;
		}*/
...
//=====================================================================
function cs_get_add_packages( $directory_featured ='' , $cs_directory_pkg_names ='' ){
	...
	?>
    <li class="enable-post" style="display:none;">  
        <div class="cs-package-lower">
            ...
            ?>
            <div class="inner-sec">
                <p class="cs-package-desc"><?php _e('Have your Ad appear at the top of the category listings for 3, 7 or 14 days.', 'directory'); ?></p>
                <div class="cs-packges">
                    <script type="text/javascript">
                        jQuery(document).ready(function ($) {
                            window.asd = $('.cs_packages_select').SumoSelect();
                    ...
                    <select class="cs_packages_select" onchange="cs_package_type(this.value)" name="dir_cusotm_field[cs_directory_pkg_names]" data-package="<?php echo intval( isset( $cs_directory_pkg_names ) ? $cs_directory_pkg_names : '' );?>" data-post="<?php // echo intval( $post_id );?>">
                        <!--Free Package-->
                        <?php if( isset( $cs_free_package_switch ) && $cs_free_package_switch == 'on' ) {?>
                        <option <?php echo esc_attr( $cs_free_selected );?> value="<?php echo '0000000000';?>" id="choos-package-<?php echo esc_attr( '0000000000' );?>"><?php echo esc_attr( 'UNLIMITED' );?> - <?php echo esc_attr( 'Free' );?></option>
                        <?php }?>
                        <!--Free Package-->
                        <?php
                        if(isset($cs_packages_options) && is_array($cs_packages_options) && count($cs_packages_options)>0){
                            ...
                            foreach($cs_packages_options as $package_key=>$package){
                                if(isset($package_key) && $package_key <> ''){
                                    ...
                                    if( $package_id <> '' && $package_title <> '' ){
                                        ...
                                        ?>
                                        <option <?php echo esc_attr( $selected );?> value="<?php echo intval( $package_id );?>" id="choos-package-<?php echo intval( $package_id );?>"><?php echo esc_attr( $package_title );?> - <?php echo intval( $package_duration );?><?php _e('days','directory');?> - <?php echo esc_attr( $currency_sign.$package_price )?></option>
                                        <?php
                                    }
                                }
                            }
        ...
        <div class="cs-package-upper">
            <span class="cs-feature-package"><?php _e('Urgent','directory');?></span>
            <div class="inner-sec">
                <?php
                if( isset( $directory_feature_price ) && $directory_feature_price <> '' )
                    echo cs_allow_special_char($currency_sign.$directory_feature_price);
                else
                    echo cs_allow_special_char($currency_sign.$cs_theme_options['directory_featured_ad_price']);
                ?>
                ...
                <ul class="radio-box">
                    <li>
                        <input type="radio" <?php if( isset( $directory_featured ) && $directory_featured == 'yes' ) { echo 'checked="checked"'; }?> name="dir_cusotm_field[directory_featured]" onclick="cs_package_amount_sum('<?php echo admin_url('admin-ajax.php');?>', '<?php echo $cs_directory_feature_price; ?>', '<?php echo esc_attr($currency_sign); ?>', 'yes')" id="directory_featured_yes" value="yes" /><label for="directory_featured_yes">Yes</label> 
                    </li>
                    <li>
                        <input type="radio" <?php if( ( isset( $directory_featured ) && $directory_featured == 'no' ) || $directory_featured == '' ) { echo 'checked="checked"'; }?> name="dir_cusotm_field[directory_featured]"  onclick="cs_package_amount_sum('<?php echo admin_url('admin-ajax.php');?>', '<?php echo $cs_directory_feature_price; ?>', '<?php echo esc_attr($currency_sign); ?>', 'no')" id="directory_featured_no" value="no" /><label for="directory_featured_no">No</label>          
                    </li>
                </ul>
            </div>
        </div>
